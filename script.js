    ...
    handleRouteChange() {
        ...
        if (hash) {
            ...
            if (target) {
                ...
                setTimeout(() => {
                    target.focus();
                    this.announceToScreenReader(`Navigated to ${target.textContent || target.id}`);
                }, 300);
            }
        }
    }
...
class Portfolio {
    ...
    handleFormSubmission(form) {
        ...
        if (!isValid) {
            this.focusManager.announceToScreenReader('Please correct the errors in the form');
            return;
        }
        ...
        links.forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                if (target) {
                    ...
                    if (this.isMobile()) {
                        // On mobile, use native snap scrolling
    ...
    navigateToNextSection() {
        ...
        if (currentIndex < sections.length - 1) {
            ...
            // Update progress indicator instead of nav items
    ...
    updateProgressIndicator() {
        ...
        console.log(`Current section: ${currentSection}, scroll: ${window.scrollY}`);
    ...
    // Mobile optimization - removed popup functionality as requested
    setupMobilePopups() {
        if (!this.isMobile()) return;
        
        // Setup project popups only
        this.setupProjectPopups();
        
        // Skills are now displayed directly on cards, no popups needed
    }

    // Setup project popups
    setupProjectPopups() {
    ...
    // Skills are now displayed directly on cards, no popups needed
    // setupSkillsPopups() function removed
    ...
    showProjectPopup(index) {
        const projects = [
            ...
            },
            {
                ...
                description: "A weather dashboard that displays current weather conditions and forecasts for multiple locations with interactive maps and charts.",
        ...
        this.createPopup('project', project.title, this.generateProjectPopupContent(project));
    }

    // Skills are now displayed directly on cards, no popup needed
    // showSkillsPopup() function removed
    ...
    // Skills are now displayed directly on cards, no popup content generation needed
    // generateSkillsPopupContent() function removed
    ...
    optimizeContentForMobile() {
        if (this.isMobile()) {
            ...
            // Don't hide projects - let CSS handle the layout
            // Projects will be shown in a scrollable container on mobile

            // Optimize timeline items for mobile
            const timelineDescriptions = document.querySelectorAll('.timeline-description p');
    ...
    optimizeForMobile() {
        if (this.isMobile()) {
            ...
            const heavyAnimations = document.querySelectorAll('.floating-icon, .profile-avatar');
...
// Additional CSS for form validation and accessibility
const additionalStyles = `
    ...
    .animated-smile {
        filter: drop-shadow(0 2px 4px rgba(255, 184, 71, 0.3));
    }
    ...
    [data-theme="dark"] .floating-icon:hover i {
